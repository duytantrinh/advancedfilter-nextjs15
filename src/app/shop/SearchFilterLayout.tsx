...
import { ProductsSort } from "@/wixAPI/productsContext";
...
export default function SearchFilterLayout({
  ...
  function updateFilters(updates: Partial<typeof optimisticFilters>) {
    ...
    // add all filters into Array
    Object.entries(newState).forEach(([key, value]) => {
      newSearchParams.delete(key);

      // check each filter is array or single
      if (Array.isArray(value)) {
        value.forEach((v) => newSearchParams.append(key, v)); // combine all filter + search query
      } else if (value) {
        newSearchParams.set(key, value);
      }
    });

    // start from page = 1 for new filter
