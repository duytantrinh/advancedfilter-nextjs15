...
// showing the first media (image OR video) od product
export default function ProductMedia({ media }: ProductMediaProps) {
  const [selectedMedia, setSelectedMedia] = useState(media?.[0]);

  // WHEN select option, ==> load image for that option
  useEffect(() => {
  ...
  return (
    // css for sticky left image/video
...
function MediaPreview({ mediaItem, isSelected, onSelect }: MediaPreviewProps) {
  ...
  return (
    <div
      className={cn(
        "relative my-3 cursor-pointer bg-secondary",
